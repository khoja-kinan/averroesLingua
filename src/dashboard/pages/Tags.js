 ...
 */ import { AllTagsUrl, NewTagsUrl } from "../constants/urls";
...
export default function Categories() {
  ...
  const [openNewCountry, setOpenNewCountry] = useState(false);
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = TagsList.map((n) => n.name);
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - TagsList.length) : 0;
  ...
  const TABLE_HEAD = [
    {
      id: "name",
      label: t("description.tagsPageTagname"),
  ...
  /* 
      New Specialization
  */
  const handleClickOpenNewCountry = () => {
    setOpenNewCountry(true);
  };

  const handleCloseNewCountry = () => {
    setOpenNewCountry(false);
  };
  ...
  const handleAddNew = () => {
    ...
    axios
      .post(`${NewTagsUrl}/create`, data, { headers })
      .then((response) => {
  ...
  return (
    <Page title={t("description.sideTags")}>
      <Container>
        <Stack
          direction="row"
          alignItems="center"
        ...
        >
          <Typography variant="h4" gutterBottom>
            {t("description.sideTags")}
          </Typography>
          {roles.includes(27) && (
            <Button variant="contained" onClick={handleClickOpenNewCountry}>
        ...
        <Dialog
          ...
          open={openNewCountry}
          onClose={handleCloseNewCountry}
        >
          <DialogTitle>{t("description.TagPageNewButton")}</DialogTitle>
          ...
          <DialogActions sx={{ justifyContent: "center" }}>
            <Button onClick={handleCloseNewCountry}>
        ...
        <Card>
          <UserListToolbar
            placeHolder={t("description.tagsPageSearchPlaceholder")}
